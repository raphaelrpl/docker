...
if [ "$#" -eq $SDB_INSTANCES ]; then
  ...
  find $SDB_INSTANCES_PATH -name $file_name
  sleep 1
  ...
  for f in "$@"; do
    ...
    echo "Running SciDB query..."
    iquery -naq "insert(redimension(input($SDB_1D_SCHEMA, '$SDB_INSTANCES_PATH/0/0/p', -2, $SDB_FORMAT, 0, shadowArray), $SDB_3D_ARRAY), $SDB_3D_ARRAY)"
    echo "Deleting file..."
